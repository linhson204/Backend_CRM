    ...
    try {
      ...
      switch (parsedData.type) {
        ...
        case 'comment_byB':
          ...
          const commentTargetCRM = clients.get(parsedData.to);
          if (commentTargetCRM && commentTargetCRM.readyState === WebSocket.OPEN) {
            const commentData = {
              type: 'comment_byB',
              postId: parsedData.postId,
              content: parsedData.content,
              linkUserComment: parsedData.linkUserComment || '',
              authorId: parsedData.authorId || ws.clientId,
              authorName: parsedData.authorName || 'Anonymous',
              URL: parsedData.URL || '',
            ...
            commentTargetCRM.send(JSON.stringify(commentData));
            console.log(`Đã gửi comment đến client ${parsedData.to}`);
        ...
        case 'reply_comment_byB':
          ...
          const replyTargetCRM = clients.get(parsedData.to);
          if (replyTargetCRM && replyTargetCRM.readyState === WebSocket.OPEN) {
            const replyCommentData = {
            ...
            replyTargetCRM.send(JSON.stringify(replyCommentData));
            console.log(`Đã gửi reply_comment đến client ${parsedData.to}`);
        ...
        case 'crawl_comment':
          ...
          const crawlCommentTargetClient = clients.get(parsedData.to);
          if (crawlCommentTargetClient && crawlCommentTargetClient.readyState === WebSocket.OPEN) {
            ...
            crawlCommentTargetClient.send(JSON.stringify(crawlCommentResultData));
            console.log(`Đã gửi ${parsedData.type} đến client ${parsedData.to}`);
